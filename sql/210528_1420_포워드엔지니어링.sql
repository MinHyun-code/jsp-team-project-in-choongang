...
COMMENT ON TABLE REQUEST IS '지원';

COMMENT ON COLUMN REQUEST.m_id IS '회원아이디';

COMMENT ON COLUMN REQUEST.rc_num IS '채용공고번호';

COMMENT ON COLUMN REQUEST.r_num IS '이력서번호';

COMMENT ON COLUMN REQUEST.rq_date IS '지원날짜';
...
ALTER TABLE REQUEST
	ADD
		CONSTRAINT FK_RESUME_TO_REQUEST
		FOREIGN KEY (
			r_num
		)
		REFERENCES RESUME (
...
ALTER TABLE REQUEST
	ADD
		CONSTRAINT FK_RECRUIT_TO_REQUEST
		FOREIGN KEY (
			rc_num
		)
		REFERENCES RECRUIT (
