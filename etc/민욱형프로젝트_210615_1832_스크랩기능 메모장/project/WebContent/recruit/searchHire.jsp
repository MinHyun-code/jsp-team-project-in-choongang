...
<style type="text/css">
   ...
   #ang2:nth-child(even)  { background-color:rgb(242 215 250/ 10%);  }
   #ang2:nth-child(odd)   { background-color: white; }
...
<!-- 							<li id="asideMenuMyTok"><a href="#">내 질문 보기</a></li> -->
  ...
  <div style="text-align: center;">
		<c:if test="${startPage > blockSize }">
			<a href='${pageContext.request.contextPath}/searchHire.do?pageNum=${startPage-blockSize}'>[이전]</a>
		</c:if>
		<c:forEach var="i" begin="${startPage}" end="${endPage}">
			<a href='${pageContext.request.contextPath}/searchHire.do?pageNum=${i}'>[${i}]</a>
		</c:forEach>
		<c:if test="${endPage < pageCnt }">
			<a href='${pageContext.request.contextPath}/searchHire.do?pageNum=${startPage+blockSize}'>[다음]</a>
		</c:if>
	 </div>
