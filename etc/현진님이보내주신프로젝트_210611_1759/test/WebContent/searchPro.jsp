				...
				<tr>
					<td>
						<c:if test="${startPage > blockSize }">
							<a href='searchPro.do?pageNum=${startPage-blockSize}'>[이전]</a>
						</c:if>
						<c:forEach var="i" begin="${startPage}" end="${endPage}">
							<a href='searchPro.do?cate_place=${cate_place }&cate_goal=${cate_goal }&cate_ride=${cate_ride }&cate_with=${cate_with }&kind=${board_no }&search_text=${search_text}&pageNum=${i}'>${i} </a>
						</c:forEach>
						<c:if test="${endPage < pageCnt }">
							<a href='searchPro.do?pageNum=${startPage+blockSize}'>[다음]</a>
						</c:if>
					</td>
				</tr>
			</table>
